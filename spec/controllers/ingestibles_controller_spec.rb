...
describe IngestiblesController do
  ...
  describe '#advanced_collection_search' do
    let!(:authority1) { create(:authority, :published) }
    let!(:authority2) { create(:authority, :published) }
    let!(:volume1) { create(:collection, collection_type: :volume, title: 'שירי הלל') }
    let!(:volume2) { create(:collection, collection_type: :volume, title: 'שירי אהבה') }
    let!(:periodical) { create(:collection, collection_type: :periodical, title: 'העת') }
    let!(:series) { create(:collection, collection_type: :series, title: 'סדרת מסות') }
    ...
    context 'when searching by title' do
      it 'returns collections matching the title substring' do
        get :advanced_collection_search, params: { title: 'שירי' }

        expect(response).to be_successful
        json = JSON.parse(response.body)
        expect(json.length).to eq(2)
        titles = json.map { |c| c['title'] }
        expect(titles).to contain_exactly('שירי הלל', 'שירי אהבה')
      end

      it 'returns empty array when no matches' do
        get :advanced_collection_search, params: { title: 'nonexistent' }
    ...
    context 'when searching by collection type' do
      it 'returns only collections of specified type' do
        get :advanced_collection_search, params: { types: ['volume'] }

        expect(response).to be_successful
        json = JSON.parse(response.body)
        expect(json.length).to eq(2)
        types = json.map { |c| c['type'] }
        expect(types).to all(eq('volume'))
      end

      it 'returns collections of multiple types' do
        get :advanced_collection_search, params: { types: %w(volume periodical) }

        expect(response).to be_successful
        json = JSON.parse(response.body)
        expect(json.length).to eq(3)
        types = json.map { |c| c['type'] }
        expect(types).to contain_exactly('volume', 'volume', 'periodical')
      end
    end

    context 'when searching by authority' do
      it 'returns collections directly associated with the authority' do
        get :advanced_collection_search, params: { authority_id: authority1.id }

        expect(response).to be_successful
        json = JSON.parse(response.body)
        expect(json.length).to eq(1)
        expect(json.first['title']).to eq('שירי הלל')
      end

      it 'returns collections inheriting authority from parent' do
        # Create a child collection nested in volume1 (which has authority1)
        child_collection = create(:collection, collection_type: :series, title: 'פרק א')
        create(:collection_item, collection: volume1, item: child_collection)
        ...
        json = JSON.parse(response.body)
        titles = json.map { |c| c['title'] }
        expect(titles).to include('פרק א', 'שירי הלל')
      end
    end

    context 'when combining multiple filters' do
      it 'returns collections matching all criteria' do
        get :advanced_collection_search, params: {
          title: 'שירי',
          types: ['volume'],
      ...
      it 'returns empty when filters exclude all results' do
        get :advanced_collection_search, params: {
          title: 'שירי',
          types: ['periodical']
        }
    ...
    context 'when no filters provided' do
      it 'returns all collections limited to 100' do
        get :advanced_collection_search, params: {}
