...
describe 'Ingestible copyright logic during ingestion' do
  ...
  describe 'determining intellectual property for Expression' do
    context 'when text has all public domain authorities' do
      it 'should be set to public_domain' do
        text_authorities = [
    ...
    context 'when text has copyrighted authorities' do
      it 'should use TOC value when provided' do
        text_authorities = [
      ...
      it 'should use ingestible default when TOC value is blank' do
        text_authorities = [
          { seqno: 1, authority_id: copyrighted_authority.id, authority_name: copyrighted_authority.name, role: 'author' }
        ].to_json
      ...
      it 'should default to by_permission when no value provided' do
        text_authorities = [
          { seqno: 1, authority_id: copyrighted_authority.id, authority_name: copyrighted_authority.name, role: 'author' }
        ].to_json
