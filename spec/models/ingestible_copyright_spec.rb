...
describe Ingestible, 'copyright calculation' do
  ...
  describe '#calculate_copyright_status' do
    context 'when all authorities are public domain' do
      it 'returns public_domain' do
        ingestible.default_authorities = [
          { seqno: 1, authority_id: public_domain_authority.id, authority_name: public_domain_authority.name, role: 'author' }
        ].to_json
    ...
    context 'when at least one authority is copyrighted' do
      it 'returns copyrighted' do
        ...
        text_authorities = [
          { seqno: 1, authority_id: copyrighted_authority.id, authority_name: copyrighted_authority.name, role: 'translator' }
        ].to_json
    ...
    context 'when text overrides role with copyrighted authority' do
      it 'returns copyrighted' do
        ...
        text_authorities = [
          { seqno: 1, authority_id: copyrighted_authority.id, authority_name: copyrighted_authority.name, role: 'author' }
        ].to_json
    ...
    context 'when collection authorities include copyrighted authority' do
      it 'returns copyrighted' do
        ingestible.collection_authorities = [
          { seqno: 1, authority_id: copyrighted_authority.id, authority_name: copyrighted_authority.name, role: 'editor' }
        ].to_json
        ingestible.default_authorities = [
          { seqno: 1, authority_id: public_domain_authority.id, authority_name: public_domain_authority.name, role: 'author' }
        ].to_json
  ...
  describe '#merge_authorities_per_role' do
    it 'merges work and default authorities per role' do
      default_auths = [
        { seqno: 1, authority_id: public_domain_authority.id, authority_name: public_domain_authority.name, role: 'author' },
        { seqno: 2, authority_id: copyrighted_authority.id, authority_name: copyrighted_authority.name, role: 'translator' }
      ].to_json
