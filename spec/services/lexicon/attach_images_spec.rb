...
describe Lexicon::AttachImages do
  ...
  let(:html) do
    <<~HTML
      ...
      <img id="missing" src="00123_files/gone.png"/>
      <img id="srcless"/>
    HTML
  end
  ...
  before do
    ...
    allow(Lexicon::MigrateAttachment).to receive(:call).with('00123_files/gone.png', lex_entry).and_return(nil)
  ...
  # Left relative it would be resolved against whatever page the image is rendered on
  it 'sends an image it could not migrate back to the old lexicon site' do
    call
    expect(html_doc.at_css('#missing')['src']).to eq('https://benyehuda.org/lexicon/00123_files/gone.png')
  end

  it 'leaves an img without a src alone' do
    call
    expect(html_doc.at_css('#srcless')['src']).to be_nil
  end
end
