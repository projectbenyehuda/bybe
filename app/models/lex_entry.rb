...
class LexEntry < ApplicationRecord
  ...
  # operation: everything editors added or corrected since the original migration is lost.
  # The entry is reset to `raw` and then set to `migrating`, disappearing from the public
  # site until it is verified and published again. Deliberately kept apart from
  # #redo_migration_eligible? so that the routine verification-queue redo never picks up
  # published entries; only the files queue offers this, behind its own confirmation.
