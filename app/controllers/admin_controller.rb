...
class AdminController < ApplicationController
  ...
  def index
    ...
    if current_user.has_bit?('moderate_tags')
      @pending_tags = Tag.where(status: :pending).where('taggings_count > 0').count
  ...
  def tag_moderation
    ...
    @pending_tags = Tag.includes(:taggings).where(status: status_to_query).where('taggings_count > 0').distinct.order(:created_at)
  ...
  def tag_review
    ...
    if @tag.nil?
      ...
      calculate_editor_tagging_stats
      @next_tag_id = Tag.where(status: :pending).where('taggings_count > 0').where('created_at > ?',
                                                       @tag.created_at).order(:created_at).limit(1).pluck(:id).first
      @prev_tag_id = Tag.where(status: :pending).where('taggings_count > 0').where('created_at < ?',
  ...
  def approve_tag_and_next # to be called from tag review page (non-AJAX)
    ...
    if session[:tagging_lock]
      ...
      if t.present?
        t.approve!(current_user)
        Notifications.send_or_queue(:tag_approved, t.creator.email, t) unless t.creator.blocked? # don't send email if user is blocked
        next_items = Tag.where(status: :pending).where('taggings_count > 0').where('created_at > ?', t.created_at).order(:created_at).limit(1)
  ...
  def escalate_tag
    ...
    if session[:tagging_lock]
      ...
      if @tag.present?
        ...
        respond_to do |format|
          format.html do
            next_items = Tag.where(status: :pending).where('taggings_count > 0').where('created_at > ?',
